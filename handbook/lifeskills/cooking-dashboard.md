# Cooking Dashboard — 2026-09-01 (all storages)

Work top to bottom in each queue. Do the batch, it drops off next refresh. Every batch is a full bag — pull the exact quantities shown; buy/farm up to it if short.

## 🏪 Buy first (in storage)
Nothing this pass — Mineral Water (91k), Leavening (53k), Sugar (22k), Salt (56k), oils all well-stocked.

## 💰 Buy on market (don't cook)
| Item | Note |
|---|---|
| Red Sauce | Buy order ~7,600–8,200 when low. Have ~54k — no buy needed. |
| Hunted meat | Standing buy orders (cheap group). Have ~32k, none cooked this pass — no buy needed now. |

---

## ⚙️ Processing queue
Nothing short this pass — Wheat Flour (19k), Wheat Dough (20k), Cheese (17k), Lump (17k) all well-stocked.

---

## 🔪 Cooking queue (~10–15 min each)

### 1. Cheese Gratin _(short for meal — have 2,137 / meal needs 2,285; Pumpkin short 4,371 → fill from Napa Cabbage 30k)_
| Ingredient | Qty |
|---|--:|
| Wheat Dough | 7,545 |
| Pumpkin | 6,036 |
| Grilled Sausage | 1,509 |
| Cheese | 4,527 |
| Red Sauce | 4,527 |

### 2. Balenos Meal _(blue procs → **3,200 crafts**, not 2,285: 5 lighter items/craft fills the same bag with ~40% more meals)_
| Ingredient | Qty |
|---|--:|
| Cheese Gratin | 3,200 |
| Meat Croquette | 3,200 |
| Golden Smoked Fish Steak _(blue, 1:1)_ | 3,200 |
| Crispy Stir-Fried Veg _(blue, =2 green)_ | 3,200 |
| Cold Draft Beer _(blue, =2 green)_ | 3,200 |

_Blue recipe = 5 items/craft × 0.1 LT = 0.5 LT → 1,600 ÷ 0.5 = 3,200 crafts (green's 7 items = 0.7 LT → 2,285). Veg & Beer are 2-qty slots → 1 blue = 2 green. Fish uses Golden (blue) 1:1. Gratin & Croquette stay **green** — sell the blue Chewy Gratin & Crispy Croquette. Blue stock covers 3,200 each; Croquette (3,678) barely — watch it next batch._

### 3. Carrot Confit _(have 50 / quest needs 650 — carrots stocked ✓)_
| Ingredient | Qty |
|---|--:|
| Special Carrot | 3,264 |
| Cinnamon | 6,528 |
| Lump of Raw Sugar | 4,896 |
| Mineral Water | 9,792 |
| Salt | 3,264 |

---

## 🌾 Farm today (lowest stock first)
Farm one crop/day — do the top one. Ranked by your farmed **Special** stock (magical seeds); downgrade to the recipe grade as needed.

| Crop | Have (Special) | Feeds |
|---|--:|---|
| **Special Hot Pepper** | 8,831 | Stir-Fried Veg (downgrade → HQ) |
| Special Onion | 10,131 | Grilled Sausage (downgrade → Onion) |
| Special Carrot | 10,170 | Carrot Confit + weekly quest |

_Regular Onion (3,408) is fine — top it up by downgrading your 10k Special Onion, no farming needed._
